document.addEventListener("DOMContentLoaded", async function () {
  try {
    ...
    const typeResponse = await fetch('https://api.magicthegathering.io/v1/types');
  ...
  try {
    ...
    const subtypeResponse = await fetch('https://api.magicthegathering.io/v1/subtypes');
  ...
  try {
    ...
    const formatResponse = await fetch('https://api.magicthegathering.io/v1/formats');
  ...
  document.getElementById('search-form').addEventListener('submit', async function (e) {
    ...
    try {
      ...
      if (cards.length === 0) {
        document.getElementById('card-results').innerHTML = '<p>No cards found.</p>';
      } else {
        // Dispatch custom event with search results
        const event = new CustomEvent('search-results-ready', { detail: cards });
        document.dispatchEvent(event);
      }
    } catch (error) {
      console.error('Error fetching cards:', error);
      document.getElementById('card-results').innerHTML = '<p>Error fetching cards. Please try again.</p>';
    }
  });
});